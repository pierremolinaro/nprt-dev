...
//--- Align pointer
  if ((((sinteger) ptr) % ALIGNMENT) != 0) {
    ptr = (char *) (((((sinteger) ptr) / ALIGNMENT) + 1) * ALIGNMENT) ;
    blockSize -= ALIGNMENT ;
  }
...
uint32 C_us_hashmap::getAllocatedSizeInBytes (void) {
...
uint32 C_us_hashmap::C_TreeForCollision::internalRecursiveSweep (C_us_hashmap::MyBlockavltree_element_for_collision * inElement) {
...
uint32 C_us_hashmap::C_TreeForCollision::sweepUnmarkedObjects (void) {
...
sint32 C_us_hashmap::C_TreeForCollision::
...
uint32 C_us_hashmap::sweepUnmarkedObjects (void) {
  uint32 sweepedNodes = 0 ;
...
sint32 C_us_hashmap::getMarkedNodesCount (void) const {
  sint32 count = 0 ;
  for (sint32 i=0 ; i<mEntryCurrentCount ; i++) {
