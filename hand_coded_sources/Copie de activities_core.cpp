...
#include "utilities/F_get_prime.h"
...
//---------------------------------------------------------------------------*
  ...
  class cActivityList {
    ...
    public : static void computeBestAndWorstResponseTime (TC_unique_grow_array <cResponseTime> & ioResponseTimeArray) ;
...
static sint32 ResourceMinDuration[100];
...
class cResourceSchedule {
  public : TC_unique_dyn_array <cResource2> mArray ;
...
class cScheduleMap {
  #ifdef HASH_MAP
    private : TC_unique_grow_array <cResourceSchedule *> mResourceNodesAVLtree ;
...
#ifndef FORGET_ACTIVITY_NODES
  static void
  internalComputeBestAndWorstResponseTime (TC_unique_grow_array <cResponseTime> & ioResponseTimeArray,
...
#ifndef FORGET_ACTIVITY_NODES
void cScheduleMap::
computeBestAndWorstResponseTime (TC_unique_grow_array <cResponseTime> & outResponseTimeArray,
                                 const sint32 inActivitiesCount) {
  TC_unique_grow_array <cResponseTime> otherMethodResponseTimeArray (inActivitiesCount COMMA_HERE) ;
...
WillRecieveHigherActivation(const cResourceSchedule * inPtr,
  ...
  while ( ( additionIndex < inReadyAtThisInstant.getCount())
        && 
        ( (SuccessorOffset + inCurrentInstant) >= inReadyAtThisInstant (additionIndex COMMA_HERE).mThisInstant)){    
	  //--- New indpendent activities will be released!
	  //--- Check if any one has higher priority than the possible CA
	    sint32 index = inReadyAtThisInstant (additionIndex COMMA_HERE).mActivityIndex;
	    do{
	       sint32 inActivityResource = inActivities (index COMMA_HERE).mResourceId ;
	       sint32 priority = inActivities (index COMMA_HERE).mPriority ;
	       if ( (SuccessorResourceIndex == inActivityResource)
	          &&
	          (priority < SuccessorPriority) ){
	            if ( (inReadyAtThisInstant (additionIndex COMMA_HERE).mThisInstant + 
	                 inActivities (index COMMA_HERE).mMinDuration) > ( inCurrentInstant + SuccessorOffset) ){
	  	          WillReceive = true;
	  	        }
		     }  
			    index = inActivities (index COMMA_HERE).mOtherReadyAtThisInst ;
		  }while( (index != -1) && !WillReceive );
	  additionIndex ++;
  }
      ...
      if ( inPtr->mArray (index COMMA_HERE).mExecutionCounter >=  
        ...
        if (successorIndex >= 0) {
	        ...
	        if ( (SuccessorResourceIndex == successorResource )
	           ...
	           if ( (MAX(0,inActivities (currentActivity COMMA_HERE).mMinDuration-inPtr->mArray (index COMMA_HERE).mExecutionCounter)+inActivities (successorIndex COMMA_HERE).mMinDuration)  >
	                SuccessorOffset ){
	             WillReceive = true;
	           }
	        }
	        ...
	        while ( (OtherHeirId >= 0) && !WillReceive ) {
	          successorPriority = inActivities (OtherHeirId COMMA_HERE).mPriority;
		        sint32 HeirResource = inActivities (OtherHeirId COMMA_HERE).mResourceId ;
		        if ( (SuccessorResourceIndex == HeirResource)
		           &&
	    	      (successorPriority < SuccessorPriority) ){
	             if ( (MAX(0,inActivities (currentActivity COMMA_HERE).mMinDuration-inPtr->mArray (index COMMA_HERE).mExecutionCounter)+inActivities (OtherHeirId COMMA_HERE).mMinDuration)  >
	                SuccessorOffset ){
	             WillReceive = true;
	           }
	    	    }
...
LeastBusyDuration (const cResourceSchedule * inPtr,
    ...
    do{
       ...
       if ( (inActivityResource == nextActivityResource)
          &&(priority <inActivityPriority) ){
  	      	leastBusy = leastBusy + inActivities (nextActivityIndex COMMA_HERE).mMinDuration ;
	     }  
 ...
 //--- and has a higher priority than the activity that will be added --> add to busy period
	           leastBusy = leastBusy + inActivities (successorIndex COMMA_HERE).mMinDuration ;
	          }
	        }
	        ...
	        while ( OtherHeirId >= 0 ) {
	          successorPriority = inActivities (OtherHeirId COMMA_HERE).mPriority;
		        ...
		        if ( (inActivityResource == HeirResource) 
		           &&
	    	      (successorPriority <inActivityPriority) ){
                leastBusy = leastBusy + inActivities (OtherHeirId COMMA_HERE).mMinDuration ;
	    	    }
	    	    OtherHeirId = inActivities (OtherHeirId COMMA_HERE).mOtherHeirId;
		      }
	      }   
...
HasToAddSuccessors (const cResourceSchedule * inPtr,
  ...
  if (SuccessorIndex >= 0) {
    ...
    sint32 leastBusyPeriod = 
      inPtr->mArray (SuccessorResource COMMA_HERE).mActivitiesToScheduleList.getLeastBusyPeriod (SuccessorIndex,
                     inActivities);
    ...
    if ( currentActivityOnSuccessurResource < 0 ){
      ...
      bool willReceiveHihgerPriorityOnSuccResAtThisInst = 
      	           WillRecieveHigherActivation(inPtr, inResourceCount,
			                                         ...
			                                         inAdditionIndex, inReadyAtThisInstant, 
			                                         inActivities);     
			
			if (! willReceiveHihgerPriorityOnSuccResAtThisInst){
			  HasToAdd= true;
		  } 	
    } else if( (currentActivityOnSuccessurResource >= 0)
    		 	&&
    			((inPtr->mArray (SuccessorResource COMMA_HERE).mExecutionCounter + SuccessorOffset) >=
      ...
      if (AnyReadyAt){           
        const sint32 HighestReadyAt=
	      ...
	      if ( (inActivities (HighestReadyAt COMMA_HERE).mPriority < SuccessorPriority)
	            &&
	             (inActivities (HighestReadyAt COMMA_HERE).mMinDuration > SuccessorOffset) ){
	      //--- if so do nothing
	      }else {
	      	 HasToAdd= true;
	      }
...
//--- do the same for other heirs
	  ...
	  while ( (OtherHeirId >= 0) && !HasToAdd ) {
		  ...
		  sint32 
        leastBusyPeriod = inPtr->mArray (HeirResource COMMA_HERE).mActivitiesToScheduleList.getLeastBusyPeriod (OtherHeirId,
                          inActivities);
      leastBusyPeriod = LeastBusyDuration (inPtr, leastBusyPeriod, inCurrentInstant,
		  ...
		  }else if( (currentActivityOnHeirResource >= 0)
	    		 	 &&
	    			 ((inPtr->mArray (HeirResource COMMA_HERE).mExecutionCounter + HeirOffset) >=
        ...
        if (AnyReadyAt){
          const sint32 HighestReadyAt=
	        ...
	        if ( (inActivities (HighestReadyAt COMMA_HERE).mPriority < HeirPriority)
	            &&
	             (inActivities (HighestReadyAt COMMA_HERE).mMinDuration > HeirOffset) ){
								//Do nothing !	         
	        }else{
	        	 HasToAdd= true;
	        }
...
NewReadyInfluenceOnScheduling (const cResourceSchedule * inPtr, 
  ...
  if (AnyReadyAt && !AnyInsertedButNotReady){
    ...
    if (inCurrentInstant==HPAReadyAtInstant){
	    ...
	    sint32 currentActivity = inPtr->mArray (inResourceIndex COMMA_HERE).mCurrentActivity ;
	    ...
	    MaximumBusyPeriod += 
		  ...
		  while (!WillReceiveNew 
		         &&
		         (additionIndex < inReadyAtThisInstant.getCount())
	           && 
	           (MaximumBusyPeriod >= inReadyAtThisInstant (additionIndex COMMA_HERE).mThisInstant) ){ 
	        sint32 nextActivityIndex = inReadyAtThisInstant (additionIndex COMMA_HERE). mActivityIndex;
	        do{
	          sint32 nextActivityResource = inActivities (nextActivityIndex COMMA_HERE).mResourceId ;
		  	...
		  	if( !NoInterdependenceButUseB){
			  	for (sint32 index = 0 ; (index < inResourceCount) && ! WillReceiveNew ; index++){
			  	  if (index != inResourceIndex){
			      	currentActivity = inPtr->mArray (index COMMA_HERE).mCurrentActivity ;
			  			if (currentActivity >= 0){
			  		  	sint32 CertainFutureBusyDuration = 
			  		    	inActivities (currentActivity COMMA_HERE).mMinDuration-inPtr->mArray (index COMMA_HERE).mExecutionCounter;
			  				...
			  				if (CertainFutureBusyDuration <= MaximumBusyPeriod ){
			  					...
			  					if (successorId >= 0){
			  						...
			  						if (successorResource == inResourceIndex){
			  							...
			  							if( (inActivities (successorId COMMA_HERE).mMinDuration + CertainFutureBusyDuration) <= MaximumBusyPeriod  ){
			  								WillReceiveNew = true;
			  							}else{
	                        ...
	                        if (HeirResource == inResourceIndex){
			  						      	...
			  						      	if( (inActivities (OtherHeirId COMMA_HERE).mMinDuration + CertainFutureBusyDuration) <= MaximumBusyPeriod  ){
			  								      WillReceiveNew = true;
			  							      }
			  						      }
		                      OtherHeirId = inActivities (OtherHeirId COMMA_HERE).mOtherHeirId;
			  					...
			  					if ( (ResourceMinDuration[index]+ CertainFutureBusyDuration) <= MaximumBusyPeriod ){
			  				    WillReceiveNew = true;
			  					}
			  				}
			  		...
			  		}else if (DependentActivitiesHasOffset){ 
			  		  //--- If offsets of dependent are != 0
			  		  currentActivity = inPtr->mArray (index COMMA_HERE).mCurrentActivity ;
			  		  sint32 successorId = inActivities (currentActivity COMMA_HERE).mSuccessorId;
	  					if (successorId >= 0){
	  						sint32 successorResource = inActivities (successorId COMMA_HERE).mResourceId;
	  						if (successorResource == inResourceIndex){
	  						  if (inActivities (successorId COMMA_HERE).mOffset != 0){
	  								WillReceiveNew = true;
	  							}
	  						}else{
	  						  sint32 OtherHeirId = inActivities (successorId COMMA_HERE).mOtherHeirId ;
			    ...
			    //--- Test if offsets of dependent are != 0
			    currentActivity = inPtr->mArray (inResourceIndex COMMA_HERE).mCurrentActivity ;
			  	sint32 successorId = inActivities (currentActivity COMMA_HERE).mSuccessorId;
	  			if (successorId >= 0){
	  			  if (inActivities (successorId COMMA_HERE).mOffset != 0){
	  				  WillReceiveNew = true;
	  			  ...
	  			  sint32 OtherHeirId = inActivities (successorId COMMA_HERE).mOtherHeirId ;
						while ( (OtherHeirId >= 0 ) && !WillReceiveNew) {
  					  if (inActivities (OtherHeirId COMMA_HERE).mOffset != 0){
	  				    WillReceiveNew = true;
	  				  }
	  			    OtherHeirId = inActivities (OtherHeirId COMMA_HERE).mOtherHeirId;
...
recursiveOP (cResourceSchedule * inPtr,
             ...
             cScheduleMap & ioScheduleMap,
               ...
               TC_unique_grow_array <cResponseTime> & ioResponseTimeArray,
             #endif
             const sint32 inResourceCount) {
    ...
    if (currentActivity < 0) {
      ...
      if (! inPtr->mArray (inResourceIndex COMMA_HERE).mHasNoNewActivities){
         NewActivationDuringBusyPeriod = 
	      ...
	      inPtr->mArray (inResourceIndex COMMA_HERE).mHasNoNewActivities = ! NewActivationDuringBusyPeriod;
	    }
	    
	    if (inPtr->mArray (inResourceIndex COMMA_HERE).mHasNoNewActivities){
	      if (inPtr->mArray (inResourceIndex COMMA_HERE).mExecutionCounter == 
	         inActivities (currentActivity COMMA_HERE).mMinDuration) {
	      	NewActivationDuringBusyPeriod = true;
	      	
	        if (inPtr->mArray (inResourceIndex COMMA_HERE).mLHBound == 0 ){
      ...
      if (SuccessorId >= 0){
      	...
      	if ( DependentActivitiesHasOffset
      	    ...
      	    (inActivities (SuccessorId COMMA_HERE).mOffset != 0) ){
	  				NotZeroOffsetOnSameResource = true;
	  		}else if(DependentActivitiesHasOffset){
	  		  sint32 OtherHeirId = inActivities (SuccessorId COMMA_HERE).mOtherHeirId ;
					while ( (OtherHeirId >= 0 ) && ! NotZeroOffsetOnSameResource) {
  				  sint32 HeirResource = inActivities (OtherHeirId COMMA_HERE).mResourceId ;
            if ( (HeirResource == inResourceIndex)
                 &&
                 (inActivities (OtherHeirId COMMA_HERE).mOffset != 0) ){
	  						NotZeroOffsetOnSameResource = true;
	  				}
	  				OtherHeirId = inActivities (OtherHeirId COMMA_HERE).mOtherHeirId;
	  			}
...
scheduleActivities (const sint32 NoInterButUseB,
                    ...
                    const TC_unique_grow_array <cReadyAtThisInstant> & ReadyAtThisInstant, 
                    const sint32 inResourceCount,
                    const TC_unique_grow_array <cActivity> & inActivities,
  ...
  const sint32 NumberOfIndependentInsertion =  ReadyAtThisInstant.getCount();
